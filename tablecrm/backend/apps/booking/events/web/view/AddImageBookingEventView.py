...
class AddImageBookingEventView:
    ...
    async def __call__(
        self,
        token: str, add_photos: List[AddBookingEventPhotoModel]
    ):
        user = await get_user_by_token(token)

        await self.__booking_events_service.add_photos(
            events_photo=add_photos,
